-- Comando 1: Criar a base de dados e prepará-la para uso.
...
-- Comando 2: Listar todos os autores cujo nome inicia com a letra ‘A’
SELECT Nome
FROM Autor
WHERE Nome LIKE 'A%';

-- Comando 3: Listar todos os livros cujo título contenha a palavra ‘sistema’
SELECT Titulo 
FROM Livro
WHERE Titulo LIKE '%Sistema%';

-- Comando 4: Listar a chave primária e o título dos livros que foram publicados há mais de 5 anos
SELECT Id, DtaPublicacao
FROM Livro
WHERE DtaPublicacao < CURDATE() - INTERVAL 5 YEAR;

-- Comando 5: Listar, em ordem alfabética por título, os livros que possuem menos de 5 exemplares disponíveis
SELECT Titulo, Exemplares
FROM Livro
WHERE Exemplares < 5
ORDER BY Titulo ASC;

-- Comando 6: Listar todos os livros que nunca foram emprestados
SELECT Titulo
FROM Livro
WHERE Id NOT IN (SELECT LivroId FROM Emprestimo);

-- Comando 7: Mudar a data de devolução real de todos os empréstimos para a data atual
UPDATE Emprestimo
SET DtaDevolucao = CURDATE();

-- Comando 8: Excluir todos os membros que nunca fizeram empréstimo
DELETE FROM Membro
WHERE Id NOT IN (SELECT DISTINCT MembroId FROM Emprestimo);

-- Comando 9: Listar o título do livro e o nome do autor para todos os livros cadastrados na base
SELECT L.Titulo, A.Nome
FROM Livro L
INNER JOIN Autor A
ON L.AutorId = A.Id;

-- Comando 10: Listar a data de empréstimo, o nome do membro que emprestou, e o título do livro de todos os empréstimos feitos neste ano
...
-- Comando 11: Listar o nome da categoria e o título do livro de todos os livros cadastrados na base
SELECT C.Genero, L.Titulo
FROM Livro L
INNER JOIN Categoria C
ON L.Categoria = C.Id;

-- Comando 12: Listar o título do livro, a data de empréstimo e a data da devolução real de todos os livros da base
SELECT L.Titulo, E.DtaEmprestimo, E.DtaDevolucao
FROM Livro L
INNER JOIN Emprestimo E
ON L.Id = E.LivroId;

-- Comando 13: Listar a data de empréstimo, data da devolução real, nome do membro que emprestou, título do livro, nome da categoria e nome do autor (ou autores) de todos os empréstimos realizados
...
-- Comando 14: Contar quantos livros estão cadastrados na base
SELECT COUNT(Id) AS ContagemdeLivros
FROM Livro;

-- Comando 15: Contar quantos empréstimos foram feitos no ano passado
SELECT COUNT(Id) AS ContagemdeEmprestimos
FROM Emprestimo
WHERE YEAR(DtaEmprestimo) = YEAR(CURDATE()) - 1;

-- Comando 16: Listar o nome da categoria e a quantidade de livros por categoria
...
-- Comando 17: Listar o título do livro e o nome do membro de todos os livros emprestados na semana corrente, agrupados e ordenados por data de empréstimo
...
-- Comando 18: Listar o total de livros emprestados no ano atual, agrupados e ordenados cronologicamente por mês, sendo que o nome do mês deve ser apresentado por extenso
